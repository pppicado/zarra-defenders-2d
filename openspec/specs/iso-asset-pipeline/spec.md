...
### Requirement: ASSET-004 — Asset loading (F3 catalog — tiles + enemy roster + hand_pen)
...
| `hand_pen` | `assets/sprites/hand_pen.png` | active | n/a | Real asset generated in F3 via `minimax_text_to_image` per ASSET-001 / ASSET-011 conventions. |
...
(Previously F2.5.2: the bootstrap loaded 35 tiles; F3 expands the asset surface to tiles + enemy roster + hand sprite. The "35 tiles" text in the prior ASSET-004 is OUT OF DATE — the live count is 40 tiles per `iso-tile-system` TILE-001 / TILE-002.)
...
The prompt SHALL NOT reference "diamond", "2:1 ratio", or any non-square shape language. After generation, the file SHALL be post-processed with `tools/postprocess-hand.py` to (a) crop to the hand's bounding box, (b) resize NEAREST back to 64×64, (c) quantize to ~16 colors, and (d) replace the magenta background with transparent alpha.
...
#### Scenario: Minimax invocation is one-shot for hand_pen

- GIVEN the F3 PR includes one and only one `minimax_text_to_image` call for the hand sprite
- WHEN `git grep minimax_text_to_image` runs on the F3 commit
- THEN exactly one match exists for the hand sprite generation (the `hand_pen` artifact)
