...
**Capability**: iso-tile-system (MODIFIED)

## Purpose

Replace the F2.5.1 2:1 diamond tile shape with a 64×64 square texture rotated 45° at the container level (Habbo / FarmVille-style "square iso falso"). The transform math pivots to a square isometric ratio (`hh = tileSize/2` on both axes); the on-disk PNG stays top-down so the gallery can toggle between top-down and 45° views without re-rasterising.

## ADDED Requirements

None — pure pivot; no new behaviour outside MODIFIED block.
...
- AND `_tileLayer.rotation = Math.PI / 4`
...
- GIVEN `_tileLayer.rotation = Math.PI / 4` is active
